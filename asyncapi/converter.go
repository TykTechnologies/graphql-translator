...
// ImportParsedAsyncAPIDocument imports a parsed AsyncAPI document and returns a GraphQL Document.
// The function creates a converter instance and initializes the data structure for introspection.
...
// ImportAsyncAPIDocumentByte reads a byte array containing an AsyncAPI document and returns
// the parsed document as an ast.Document and an operationreport.Report. It first calls
// ParseAsyncAPIDocument to parse the input byte array into an AsyncAPI struct. If parsing fails,
// an internal error is added to the report and nil values are returned. Otherwise, it calls
// ImportParsedAsyncAPIDocument to convert the parsed AsyncAPI struct into an ast.Document using a converter.
// The resulting ast.Document and the report are returned.
...
// ImportAsyncAPIDocumentString imports an AsyncAPI document from a string input.
// It internally calls ImportAsyncAPIDocumentByte to parse and import the document.
// The input is expected to be a valid AsyncAPI document in string format.
// It returns the imported document as *ast.Document and an operationreport.Report.
// If there are any parsing or importing errors, the report will contain the error details.
